...
, val: boolean
  ...
  lazyStatic(() => getDatabase().prepare(`
    INSERT INTO estore_token_policy (namespace, write_token_required)
    VALUES ($namespace, $writeTokenRequired)
        ON CONFLICT(namespace)
        DO UPDATE SET write_token_required = $writeTokenRequired;
  `), [getDatabase()]).run({
    namespace
  , writeTokenRequired: booleanToNumber(val)
  })
})

export const unsetWriteTokenRequired = withLazyStatic(function (
  namespace: string
): void {
  lazyStatic(() => getDatabase().transaction((namespace: string) => {
    lazyStatic(() => getDatabase().prepare(`
      UPDATE estore_token_policy
...
, val: boolean
  ...
  lazyStatic(() => getDatabase().prepare(`
    INSERT INTO estore_token_policy (namespace, read_token_required)
    VALUES ($namespace, $readTokenRequired)
        ON CONFLICT(namespace)
        DO UPDATE SET read_token_required = $readTokenRequired;
  `), [getDatabase()]).run({
    namespace
  , readTokenRequired: booleanToNumber(val)
  })
})
...
, val: boolean
  ...
  lazyStatic(() => getDatabase().prepare(`
    INSERT INTO estore_token_policy (namespace, delete_token_required)
    VALUES ($namespace, $deleteTokenRequired)
        ON CONFLICT(namespace)
        DO UPDATE SET delete_token_required = $deleteTokenRequired;
  `), [getDatabase()]).run({
    namespace
  , deleteTokenRequired: booleanToNumber(val)
  })
})

export const unsetDeleteTokenRequired = withLazyStatic(function (
  namespace: string
): void {
  lazyStatic(() => getDatabase().transaction((namespace: string) => {
    lazyStatic(() => getDatabase().prepare(`
...
const deleteNoPoliciesRow = withLazyStatic(function (namespace: string): void {
  lazyStatic(() => getDatabase().prepare(`
    DELETE FROM estore_token_policy
     WHERE namespace = $namespace
