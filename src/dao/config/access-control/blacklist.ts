...
export const addBlacklistItem = withLazyStatic(function (namespace: string) {
...
export const removeBlacklistItem = withLazyStatic(function (namespace: string) {
  lazyStatic(() => getDatabase().prepare(`
    DELETE FROM estore_blacklist
     WHERE namespace = $namespace;
  `), [getDatabase()]).run({ namespace })
})
